...
export const transformer: <T extends ts.Node>(
  program: ts.Program,
) => ts.TransformerFactory<T> = program => context => {
  const visit: ts.Visitor = node => {
    if (
      ts.isFunctionDeclaration(node) &&
      node.name &&
      node.body &&
      node.name.text === 'tailFac_'
    ) {
      return convert(
        node,
        node.parameters.map(a => a),
        node.body,
        context,
      )
    }
...
function convert(
  ...
  const paramIdentifiers = pipe(parameters, A.chain(dereferenceParam))

  const newParamIdentifiers = paramIdentifiers.map(id => [
    id,
    ts.createUniqueName(id.text),
  ])
  ...
  const replaceIdentifiers: (
    ...
    if (ts.isIdentifier(node)) {
      ...
      return ts.visitEachChild(
        node,
        child => replaceIdentifiers(child),
        context,
      )
    }
  }
  const newBody = ts.visitEachChild(
    body,
    child => replaceIdentifiers(child),
    context,
  )
    ...
    if (ts.isReturnStatement(node) && node.expression) {
      ...
      if (ts.isCallExpression(ce)) {
        if (
          ...
          func.name &&
          ce.expression.text === func.name.text
        ) {
          const tempArgs = ce.arguments.map((_, i) =>
            ts.createUniqueName(paramIdentifiers[i].text),
          )
          ...
          const commits = tempArgs.map((tempArg, i) => {
            return ts.createStatement(
              ts.createAssignment(newParamIdentifiers[i][1], tempArg),
            )
          })
  ...
  const newerBody = ts.visitEachChild(
    newBody,
    child => replaceRecursion(child),
    context,
  )
